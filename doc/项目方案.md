...
5. 支持上传图片，通过base64编码与提示词一起发送给API
6. 纯前端实现，无需后端服务
...
```javascript
async function generateImage(prompt, imageBase64, apiKey, apiEndpoint, useProxy, proxyUrl, ratio) {
...
```javascript
async function generateBatch(prompt, imageBase64, count, apiKey, apiEndpoint, useProxy, proxyUrl, ratio) {
  const promises = [];
