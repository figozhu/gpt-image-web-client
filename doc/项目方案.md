...
```javascript
async function generateImage(prompt, apiKey, apiEndpoint, useProxy, proxyUrl, ratio) {
...
```javascript
async function generateBatch(prompt, count, apiKey, apiEndpoint, useProxy, proxyUrl, ratio) {
  const promises = [];
